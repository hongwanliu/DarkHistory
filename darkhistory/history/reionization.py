...
def photoion_rate(species, method = 'Puchwein'):
    ...
    if method == 'Puchwein':
        ...
        rate_vec = load_data('reion')[method][species+'_ion']+1e-30
...
def photoheat_rate(species, method='Puchwein'):
    """ Photoheating rate. 
...
def HI_post_reion_photoheating(alpha, T, rs):
    ...
    caseA_coeffs = np.squeeze(np.array([alphaA_recomb('HII', T), alphaA_recomb('HeII', T)]))
