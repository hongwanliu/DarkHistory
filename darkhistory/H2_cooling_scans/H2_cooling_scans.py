...
output_dir = '/scratch/gpfs/hongwanl/DarkHistory/H2_cooling/'
...
for pri in ['elec', 'phot']:
    if pri == 'phot':
        mDM_arr = np.logspace(4.01, 12.78, num=20)
    else:
        mDM_arr = np.logspace(6.01, 12.78, num=20)
    
    # Array of lifetimes for decay; cross-sections for swave and pwave
    for DM_process in ['decay', 'swave', 'pwave']: 
        if DM_process == 'decay':
            struct_boost = None
            inj_par_array = np.logspace(22, 30, num=30)
        elif DM_process == 'swave':
            struct_boost = phys.struct_boost_func(model='NFW_no_subs')
            inj_par_array = np.logspace(-33, -23, num=30)
        else:
            struct_boost = phys.struct_boost_func(model='pwave_NFW_no_subs')
            inj_par_array = np.logspace(-30, -14, num=30)
...
embarrassingly_parallel_collapse(
    params_list, idx, options_dict, output_dir, 'nmax_100_Puchwein_H2_cooling'
)
